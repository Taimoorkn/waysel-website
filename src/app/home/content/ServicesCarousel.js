...
const ServiceCard = ({ name, description, icon, backgroundImage, path, onCardHover, onCardUnhover }) => {
  const [isHovered, setIsHovered] = useState(false);
  const router = useRouter();

  return (
    <div
      className="relative h-[400px] sm:h-[460px] p-6 sm:p-10 bg-primary_bg rounded-3xl flex flex-col items-center justify-between shadow-lg cursor-pointer"
    ...
    >
      ...
      <div className="flex flex-col items-start h-full text-primary_text relative">
        <div className="mb-6 p-2 sm:p-3 rounded-full border border-white text-white">{icon}</div>
        <div className="flex flex-col h-full justify-between">
          <div>
            <h3 className="font-clashGrotesk font-medium text-lg sm:text-2xl leading-tight sm:leading-[29.5px] mb-2 text-white">
              {name}
            </h3>
            <p className="font-manrope text-base sm:text-xl leading-snug sm:leading-[27.3px] text-white">
              {description}
            </p>
          </div>
          <div className={`hidden sm:block self-end transform transition-transform duration-300 ${isHovered ? "-rotate-45 text-accent" : "text-white"}`}>
...
const ServicesCarousel = () => {
  const services = [
    ...
    },
    {
      ...
      description:
        "Reliable and scalable cloud-based services for your business.",
  ...
  return (
    <section ref={sectionRef} className="bg-secondary_bg section_padding">
      ...
      <div className="relative">
        ...
        <div className="flex justify-center items-center gap-4 pb-8 mt-6 sm:mt-8">
          <button
            className="swiper-button-prev p-2 rounded-full bg-primary_bg"
            style={{ boxShadow: "rgba(193,125,6,0.25) 0px 0px 16px" }}
            aria-label="Previous slide"
          >
            <CaretLeftIcon className="w-5 h-5 sm:w-6 sm:h-6 text-primary_text" />
          </button>
          ...
          >
            <CaretRightIcon className="w-5 h-5 sm:w-6 sm:h-6 text-primary_text" />
          </button>
        </div>
      </div>
      <style jsx global>{`
